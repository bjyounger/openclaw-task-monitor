...
const plugin = {
  id: "task-monitor",
  name: "Task Monitor",
  description: "监控子任务生命周期、自动重试、任务链追踪、主任务监控、exec进程监控、失败实时上报（v13 - 事件驱动架构）",
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.logger.info?.(`[task-monitor] Plugin registering (v13 - event-driven, config version: ${config.version})...`);
    ...
    api.logger.info?.('[task-monitor] Timers registered: retryChecker, cleanupChecker');
...
`;
    ...
    api.logger.info?.("[task-monitor] Plugin registration complete (v13 - event-driven)");
  },
};
